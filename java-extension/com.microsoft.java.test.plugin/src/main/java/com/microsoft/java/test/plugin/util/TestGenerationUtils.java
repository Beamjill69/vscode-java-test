...
public class TestGenerationUtils {
    ...
    private static WorkspaceEdit generateTestsFromSource(ICompilationUnit unit, ITypeBinding typeBinding,
        ...
        final String testFullyQualifiedName = getTestFullyQualifiedName(typeBinding, unit.getJavaProject(), testEntry);
        ...
        final ICompilationUnit testUnit = getTestCompilationUnit(unit, testEntry, testFullyQualifiedName);
    ...
    private static String getTestFullyQualifiedName(ITypeBinding typeBinding, IJavaProject project,
        ...
        final IPackageFragmentRoot[] packageFragmentRoots = project.findPackageFragmentRoots(testEntry);
        if (packageFragmentRoots.length > 0) {
            final IPackageFragment packageFragment = packageFragmentRoots[0].getPackageFragment(
                typeBinding.getBinaryName().substring(0, typeBinding.getBinaryName().lastIndexOf(".")));
            final ICompilationUnit testUnit = packageFragment.getCompilationUnit(
                    typeBinding.getName() + "Test" + ".java");
            if (testUnit.exists()) {
    ...
    private static ICompilationUnit getTestCompilationUnit(ICompilationUnit unit, IClasspathEntry testEntry,
